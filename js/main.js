// === Импорты из CDN (работают в браузере) ===
...
async function startCamera() {
  try {
    const stream = await navigator.mediaDevices.getUserMedia({
      video: {
        facingMode: { ideal: "environment" }, // ← ВАЖНО: задняя камера
        width: { ideal: 1280 },
        height: { ideal: 720 }
      },
      audio: false
...
loader.load(
  ...
  (gltf) => {
    ...
    model.scale.set(0.5, 0.5, 0.5); // Подбери под свою модель (может быть 0.1, 5, и т.д.)
    scene.add(model);
...
// === 4. Обработка клика → переход на сайт ===
window.addEventListener('click', () => {
  // Замени на реальный сайт рекламодателя
  window.open('https://mirea.ru', '_blank');
});
...
function animate() {
  ...
  // Лёгкое вращение, если нет анимации (для "оживления")
...
// === 6. Адаптация под поворот экрана ===
